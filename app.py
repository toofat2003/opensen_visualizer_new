...
def load_data_from_csv_files():
    ...
    try:
        ...
        st.exception(e) # Add exception details
        return None
...
def compute_batter_stats_optimized(df):
    ...
    try:
        ...
        st.error(f"打者成績の計算中にエラーが発生しました: {e}")
        st.exception(e)
...
def calculate_stats_pitcher_optimized(df):
    ...
    try:
        ...
        st.error(f"投手成績全体の計算中にエラーが発生しました: {e}")
        st.exception(e)
        return pd.DataFrame() # エラー時は空のデータフレームを返す
...
def calculate_result_profile_pitcher_vectorized(df, pitcher=None,gametype="opensen"):
    ...
    try:
        ...
        if "TaggedPitchType" in df_pitcher.columns:
            ...
            for pitch_type in valid_pitch_types:
                ...
                for condition_name, condition_value in [("全体", None), ("2st", 2)]:
                    ...
                    if tamakazu == 0:
                        ...
                        is_foul = df_subset["PitchCall"].isin(["FoulBall","FoulBallFieldable","FoulBallNotfieldable"])
            ...
            for condition_name, condition_value in [("全体", None), ("2st", 2)]:
                ...
                if tamakazu == 0:
                    ...
                    is_foul = df_total["PitchCall"].isin(["FoulBall","FoulBallFieldable","FoulBallNotfieldable"])
        ...
        st.error(f"リザルトプロファイル計算中にエラーが発生しました: {e}")
        st.exception(e)
        return pd.DataFrame()
...
def compute_player_performance_details(df, player_name, player_type="batter"):
    ...
    try:
        ...
        st.error(f"選手詳細成績の計算中にエラーが発生しました: {e}")
        st.exception(e)
        return pd.Series() # エラー時は空のSeriesを返す
...
def compute_advanced_metrics(df, player_name, player_type="batter"):
    ...
    try:
        ...
        st.error(f"高度な指標の計算中にエラーが発生しました: {e}")
        st.exception(e) # Add exception details
        return pd.DataFrame() # エラー時は空のデータフレームを返す
                            ...
                            if not pitch_types.empty:
                                ...
                                for i, (idx, row) in enumerate(top_pitches.iterrows()):
                                    with [col1, col2, col3][i]:
                                        pitch_name = row['球種']
                                        # '全体'の球数を安全に取得し、ゼロ除算を回避
                                        overall_pitches_series = advanced_metrics[advanced_metrics['球種'] == '全体']['球数']
                                        if not overall_pitches_series.empty and overall_pitches_series.iloc[0] > 0:
                                            usage_pct = round(row['球数'] / overall_pitches_series.iloc[0] * 100, 1)
                                        else:
                                            usage_pct = 0.0 # または np.nan など適切な値
                                        ...
                                        # 変化量が有効な値（NaNでも'-'でもない）であれば表示
                                        vert_break = row['縦変化']
                                        horz_break = row['横変化']
                                        if pd.notna(vert_break) and vert_break != '-' and pd.notna(horz_break) and horz_break != '-':
                                            st.write(f"変化量: 縦{vert_break}cm / 横{horz_break}cm")
                        else:
